...
<section class="min-h-screen bg-[#F5F1EB] pb-20 pt-32">
    ...
    <div class="mx-auto max-w-[900px] px-5 sm:px-6 lg:px-10">
        ...
        <form id="carBookingForm" method="POST" action="{{ route('cars.booking.store') }}" class="rounded-[34px] bg-white p-6 shadow-lg sm:p-8">
            @csrf

            <input type="hidden" name="car_id" value="{{ $car->id }}">
            <input type="hidden" name="car_name" value="{{ $car->name }}">
            <input type="hidden" name="car_category" value="{{ $car->category }}">
            ...
            <div class="mt-4">
                ...
                <textarea
                    name="note"
                    rows="5"
                    placeholder="Pickup time, destination, driver request, or special note..."
                    class="w-full rounded-2xl border border-black/10 bg-[#F8F3EC] px-5 py-4 text-sm text-[#1A1A1A] outline-none transition duration-300 placeholder:text-[#8C7B62] focus:border-[#B07A45]">{{ old('note') }}</textarea>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const form = document.querySelector('#carBookingForm');
