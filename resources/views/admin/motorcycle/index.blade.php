                ...
                <div class="overflow-hidden rounded-[30px] bg-white shadow-[0_18px_55px_rgba(0,0,0,0.06)]">
                    ...
                    <div class="relative flex h-[230px] items-center justify-center bg-[#EEE6DA] p-6">
                        ...
                        <span class="absolute right-4 top-4 rounded-full px-3 py-1 text-[9px] font-bold uppercase tracking-[0.14em]
                            {{ $motorcycle->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                    ...
                    <div class="p-6">
                        ...
                        <p class="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-[#B07A45]">
                            {{ $motorcycle->tag ?? 'motorcycle Collection' }}
                        </p>
                        ...
                        <p class="mt-2 text-sm font-semibold text-[#B07A45]">
                            ${{ number_format($motorcycle->price_per_day) }} / Day
                        </p>
                        ...
                        <div class="mt-6 grid grid-cols-3 gap-3 border-y border-black/10 py-4 text-center text-xs">
                            <div>
                                <p class="mb-1 text-[#8C7B62]">cc</p>
                                <p class="font-semibold text-[#1A1A1A]">{{ $motorcycle->cc ?? '-' }}</p>
                            </div>

                            <div>
                                <p class="mb-1 text-[#8C7B62]">Fuel</p>
                                <p class="font-semibold text-[#1A1A1A]">{{ $motorcycle->fuel ?? '-' }}</p>
                            </div>

                            <div>
                                <p class="mb-1 text-[#8C7B62]">transmission</p>
                                <p class="font-semibold text-[#1A1A1A]">{{ $motorcycle->transmission ?? '-' }}</p>
                            </div>
                        </div>

                        <div class="mt-6 flex flex-wrap gap-2">

                            <a href="{{ route('admin.motorcycles.show', $motorcycle) }}"
                            ...
                            <form method="POST" action="{{ route('admin.motorcycles.destroy', $motorcycle) }}"
                                  onsubmit="return confirm('Yakin hapus motor ini?')">
                                @csrf
                                @method('DELETE')
